...
def create_features(input_path: str | Path, output_path: str | Path) -> pd.DataFrame | None:
    try:
        ...
        dataframe["MA_7"] = dataframe["Close"].rolling(7).mean()
        dataframe["MA_30"] = dataframe["Close"].rolling(30).mean()
        dataframe["Volatility"] = dataframe["Close"].rolling(7).std()
        ...
        print("Features created and saved to:", output_path)
        print("Latest feature row kept for API use.")
        print("Rows dropped for invalid values:", dropped_invalid_rows)
        print("Duplicate dates removed:", duplicate_dates)
        return serialized
    except Exception as error:
        print(f"Error while creating features: {error}")
        return None
